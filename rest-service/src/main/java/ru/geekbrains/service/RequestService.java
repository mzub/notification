...
public class RequestService {
	...
	private ResponseEntity<ResponseMessage> sendTaskToParsService(Task task) {
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(PARSER_SERVICE_URL).path("/task/create");
...
//        requestParam.setAnswer(answer);
	...
	private ResponseEntity<ResponseMessage> sendResponseStart(RequestParam requestParam) {
		if (requestParam != null) {
			...
			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(NOTIFICATION_SERVICE_URL).path("/callback");
			String url = builder.build().encode().toUriString();
			log.info(String.format("url = %s", url));
